...
//------------------------------
	...
	fetch('http://127.0.0.1:8085/%7B%22method%22:%22listtransactions%22,%22params%22:[]%7D').then(function(res) {
            return res.json();
        }).then(function (data1) {
        	// alert(data1.result);
			//console.log(data);
            ...
            for(var i=1;i<=data1.result.length;i++){
            			//alert(data1.result[i].address)

				  var a = {};
				  a.Ing = data1.result[i].address;
				  a.lat = data1.result[i].address;
				 // a.count = i * 10;
				  data.push(a);
